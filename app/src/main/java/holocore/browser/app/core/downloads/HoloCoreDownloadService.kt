package holocore.browser.app.core.downloads
...
class HoloCoreDownloadService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_CANCEL -> {
                ...
                if (id != -1L) {
                    HoloCoreDownloadEngine.requestCancel(id)
                }
    ...
    private fun startDownload(
        ...
        val job = serviceScope.launch {
            try {
                ...
                HoloCoreDownloadEngine.withDownloadSlot {
                    ...
                    try {
                        dao.updateStatus(downloadId, HoloCoreDownloadStatus.RUNNING.name, null, System.currentTimeMillis())
                    } catch (_: Throwable) { }

                    val destination = parseDestination(destinationUri)
                    if (destination == null) {
                        try {
                            dao.updateStatus(downloadId, HoloCoreDownloadStatus.FAILED.name, "Invalid destination", System.currentTimeMillis())
                        } catch (_: Throwable) { }
                    ...
                    val result = try {
                        HoloCoreDownloadEngine.fetch(
                            ...
                            if (now - lastDbWriteMillis > 250 || isDone) {
                                ...
                                serviceScope.launch {
                                    try {
                                        dao.updateProgress(downloadId, downloaded, total, HoloCoreDownloadStatus.RUNNING.name, System.currentTimeMillis())
                                    } catch (_: Throwable) { }
                                }
                            }
                        ...
                        when (result) {
                            is FetchResult.Success -> {
                                dao.updateProgress(
                                    ...
                                    result.bytesTotal,
                                    HoloCoreDownloadStatus.SUCCESSFUL.name,
                                    System.currentTimeMillis(),
                                )
                                notifyCompleted(downloadId, displayName)
                            }
                            is FetchResult.Failure -> {
                                dao.updateStatus(downloadId, HoloCoreDownloadStatus.FAILED.name, result.reason, System.currentTimeMillis())
                                HoloCoreDownloadEngine.deletePartialFile(applicationContext, destinationUri)
                                notifyFailed(downloadId, displayName)
                            }
                            FetchResult.Cancelled -> {
                                dao.updateStatus(downloadId, HoloCoreDownloadStatus.CANCELLED.name, null, System.currentTimeMillis())
                                HoloCoreDownloadEngine.deletePartialFile(applicationContext, destinationUri)
                ...
                try {
                    val dao = DownloadsDatabase.get(applicationContext).dao()
                    dao.updateStatus(downloadId, HoloCoreDownloadStatus.FAILED.name, e.message ?: "Download failed", System.currentTimeMillis())
                } catch (_: Throwable) { }
    ...
    private fun ensureChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Shows progress for files HoloCore is downloading"
                setShowBadge(false)
            }
    ...
    companion object {
        private const val CHANNEL_ID = "holocore_downloads"
        private const val DOWNLOAD_NOTIFICATION_GROUP = "holocore.browser.app.downloads.GROUP"
        private const val FOREGROUND_NOTIFICATION_ID = 1
        private const val ACTION_CANCEL = "holocore.browser.app.action.CANCEL_DOWNLOAD"
        ...
        fun newIntent(
            ...
            cookie: String?,
            destinationUri: String,
        ): Intent = Intent(context, HoloCoreDownloadService::class.java).apply {
        ...
        fun cancelIntent(context: Context, downloadId: Long): Intent =
            Intent(context, HoloCoreDownloadService::class.java).apply {
