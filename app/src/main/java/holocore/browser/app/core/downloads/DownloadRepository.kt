...
object DownloadRepository {
    ...
    suspend fun start(
        ...
        return try {
            ...
            val destination = HoloCoreDownloadEngine.createDestination(context, fileName, resolvedMime) ?: return null
            ...
            try {
                ContextCompat.startForegroundService(
                    context,
                    HoloCoreDownloadService.newIntent(context, id, url, userAgent, cookie, destination.uriString),
                )
            } catch (e: Throwable) {
                ...
                try {
                    ...
                    dao(context).updateStatus(id, HoloCoreDownloadStatus.FAILED.name, e2.message ?: "Could not start download", System.currentTimeMillis())
                    return null
                }
            }
    ...
    suspend fun saveBase64(
        ...
        return try {
            ...
            val record = DownloadRecord(
                ...
                status = HoloCoreDownloadStatus.SUCCESSFUL.name,
                createdAt = now,
                updatedAt = now,
            )
            val id = dao(context).upsert(record)
    ...
    suspend fun cancel(context: Context, downloadId: Long) {
        HoloCoreDownloadEngine.requestCancel(downloadId)
        context.startService(HoloCoreDownloadService.cancelIntent(context, downloadId))
    }
    ...
    suspend fun retry(context: Context, record: DownloadRecord, userAgent: String?) {
        ...
        HoloCoreDownloadEngine.deletePartialFile(context, record.destinationUri)
        val destination = HoloCoreDownloadEngine.createDestination(context, record.fileName, record.mimeType)
            ?: return
        val now = System.currentTimeMillis()
        dao(context).update(
            record.copy(
        ...
        ContextCompat.startForegroundService(
            context,
            HoloCoreDownloadService.newIntent(context, record.id, record.url, userAgent, cookie, destination.uriString),
        )
    }
