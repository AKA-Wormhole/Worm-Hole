...
class BrowserAgent(
    ...
    private suspend fun bridge(command: String, args: Map<String, String> = emptyMap()): String {
        val session = activeSession() ?: return GeckoJs.UNAVAILABLE_SENTINEL
        return try {
            holocore.browser.app.core.gecko.GeckoExtensionBridge.send(session, command, args)
        } catch (e: Throwable) {
            "ERR:${e.message}"
        }
    }
    ...
    fun tools(): BrowserToolRegistry = BrowserToolRegistry().also { registry ->
        ...
        registry.register(SimpleTool("read_page", "Extract the visible article text from the current page") {
            ...
            if (session == null) ToolResult(false, "", "No active page")
                ...
                if (isBridgeFailure(text)) {
                    text = holocore.browser.app.core.gecko.PageTranslator.readReadableText(session)
                }
        ...
        registry.register(SimpleTool("set_theme", "Set app theme. Args: mode=dark|light|system") SimpleTool@{ input ->
            val mode = when (input.arguments["mode"].orEmpty().lowercase()) {
                "light" -> holocore.browser.app.core.settings.ThemeMode.LIGHT
                "system" -> holocore.browser.app.core.settings.ThemeMode.SYSTEM
                else -> holocore.browser.app.core.settings.ThemeMode.DARK
            }
        ...
        registry.register(SimpleTool("set_search_engine", "Set the search engine by name or id") SimpleTool@{ input ->
            ...
            val engine = holocore.browser.app.core.settings.SearchEngine.entries.firstOrNull {
        ...
        registry.register(SimpleTool("create_space", "Create a browser space. Args: name") SimpleTool@{ input ->
            ...
            val space = viewModel.createSpace(name, holocore.browser.app.core.browser.SpaceAccent.VIOLET)
    ...
    private suspend fun runInternal(
        ...
        repeat(14) {
            val context = buildString {
                appendLine("You are HOLOCORE Agent with nearly full control of this Android browser.")
