package holocore.browser.app.core.crash
...
class CrashHandler private constructor(
    ...
    private fun writeCrashReport(thread: Thread, throwable: Throwable) {
        ...
        AppLog.e("Crash", "Uncaught on ${thread.name}: ${throwable.message}", throwable)
        file.writeText(
            buildString {
                appendLine("HoloCore crash report")
    ...
    companion object {
        ...
        fun recordNonFatal(context: Context, label: String, throwable: Throwable) {
            ...
            try {
                ...
                file.writeText(
                    buildString {
                        appendLine("HoloCore non-fatal report: $label")
                        appendLine("Time: $timestamp")
                        appendLine("Android: ${android.os.Build.VERSION.RELEASE} (SDK ${android.os.Build.VERSION.SDK_INT})")
                        appendLine("Device: ${android.os.Build.MANUFACTURER} ${android.os.Build.MODEL}")
                        appendLine()
