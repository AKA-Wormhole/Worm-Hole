# Knot — UI Design Brief

This is the design target for Knot's interface: an Arc-inspired Android
...
- Typing a non-URL triggers Knot's search flow (Stage 4+); typing a URL
  navigates directly.
...
- Holds the active tab's WebView (Stage 2's `KnotWebViewHost`) or, on a
...
Press feedback: every tappable element scales to `KnotMotion.PRESS_SCALE`
...
- Dark mode is a first-class target, not an afterthought: every new
  component must be checked against both `KnotDarkScheme` and
  `KnotLightScheme`.
...
1. `KnotSidebar` — collapsible container, Space switcher row, tab list
...
one source of truth for what "Arc-styled" means in Knot).
